...
const CartProvider: React.FC = ({ children }) => {
  ...
  useEffect(() => {
    async function loadProducts(): Promise<void> {
      const products = await AsyncStorage.getItem('@goMarketplace:produtcs');
  ...
  const addToCart = useCallback(async product => {
    ...
    await AsyncStorage.setItem('@goMarketplace:produtcs', JSON.stringify(products));
  }, [products]);
  ...
  const increment = useCallback(async id => {
    ...
    await AsyncStorage.setItem('@goMarketplace:produtcs', JSON.stringify(newProducts));
  }, [products]);
